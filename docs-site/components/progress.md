---
title: Progress
description: Determinate or indeterminate horizontal progress bar driven by a CSS --progress variable, with a JS API for live updates.
---

# Progress

Determinate or indeterminate horizontal progress bar driven by a CSS --progress variable, with a JS API for live updates.

<iframe
  class="brut-preview-frame"
  src="../preview/components-progress.html"
  loading="lazy"
  style="height: 720px;"
  title="Progress preview"
></iframe>

::: tip Markup source
The preview above is served from `preview/components-progress.html` —
the canonical fixture for this component. Copy markup directly from
the source to use it in your own page.
:::

## Where to look

- **Source CSS:** [`src/components.css`](https://github.com/Unal-Inanli/brut-ui/blob/main/src/components.css) (search for `.brut-progress`)
- **Manifest entry:** [`dist/components.json`](https://github.com/Unal-Inanli/brut-ui/blob/main/dist/components.json) (filter by `name`)
- **Reference:** [Manifest schema](/reference/manifest)
