---
title: Toast
description: Transient banner notification with icon slot, semantic color variants, and dismiss affordance — auto-removed by the toast host.
---

# Toast

Transient banner notification with icon slot, semantic color variants, and dismiss affordance — auto-removed by the toast host.

<iframe
  class="brut-preview-frame"
  src="../preview/components-toast.html"
  loading="lazy"
  style="height: 720px;"
  title="Toast preview"
></iframe>

::: tip Markup source
The preview above is served from `preview/components-toast.html` —
the canonical fixture for this component. Copy markup directly from
the source to use it in your own page.
:::

## Where to look

- **Source CSS:** [`src/components.css`](https://github.com/Unal-Inanli/brut-ui/blob/main/src/components.css) (search for `.brut-toast`)
- **Manifest entry:** [`dist/components.json`](https://github.com/Unal-Inanli/brut-ui/blob/main/dist/components.json) (filter by `name`)
- **Reference:** [Manifest schema](/reference/manifest)
