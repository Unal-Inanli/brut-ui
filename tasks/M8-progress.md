...
- 2026-05-05 — Row 8: package renamed `brut` → `@brut/ui`, version bumped
  ...
  `bin: { "brut": "./bin/brut.js" }` command name is unchanged — only the
  npm package name moves.
...
- 2026-05-05 — Row 6: reference/manifest.md uses VitePress include of
  ...
  changelog include).
- 2026-05-05 — Row 5: examples.md iframes the three full-page demos
  (landing, login, dashboard). Sync script extended to mirror `demos/`,
  and the CSS/JS rewriting inside synced HTML switched from absolute
  (`/brut.css`, base-dependent) to relative (`../brut.css`, base-agnostic).
- 2026-05-05 — Row 4: 51 component pages auto-generated by
  `scripts/gen-docs-component-pages.js` (re-run when preview pages
  change). Each iframes `../preview/components-<slug>.html` (relative
  path so VitePress's `base: '/brut-ui/'` is honored). Preview fixtures +
  `dist/brut.{css,js}` are synced into `docs-site/public/` by
  `scripts/sync-preview-to-docs-site.js` (registered as prebuild/predev).
  Inside the synced HTML, `../dist/brut.{css,js}` references are rewritten
  to `/brut.{css,js}`. `docs-site/public/` is gitignored.
